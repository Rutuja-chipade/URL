...
@router.get("/urls")
async def list_all_urls(skip: int = 0, limit: int = 50):
    ...
    async for doc in cursor:
        formatted = _format_url_response(doc)
...
async def bulk_upload_urls(file: UploadFile = File(...), user = Depends(get_current_admin)):
    ...
    for row in reader:
        ...
        try:
            url_res = await create_short_url(original_url=original_url, user_id=str(user["_id"]))
            results.append({
                "original_url": original_url,
